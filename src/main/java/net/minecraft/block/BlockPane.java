...
public class BlockPane extends Block {
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity) {
        if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() <= 47) {
            ...
            addCollisionBoxesToList(pos, mask, list, AABB_BY_INDEX[0]);

            if (state.getValue(NORTH))
            {
                addCollisionBoxesToList(pos, mask, list, AABB_BY_INDEX[getBoundingBoxIndex(EnumFacing.NORTH)]);
            }

            if (state.getValue(SOUTH))
            {
                addCollisionBoxesToList(pos, mask, list, AABB_BY_INDEX[getBoundingBoxIndex(EnumFacing.SOUTH)]);
            }

            if (state.getValue(EAST))
            {
                addCollisionBoxesToList(pos, mask, list, AABB_BY_INDEX[getBoundingBoxIndex(EnumFacing.EAST)]);
            }

            if (state.getValue(WEST))
            {
                addCollisionBoxesToList(pos, mask, list, AABB_BY_INDEX[getBoundingBoxIndex(EnumFacing.WEST)]);
            }
        }
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess worldIn, BlockPos pos) {
        if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() <= 47) {
            ...
            setBlockBounds(getBoundingBox(worldIn.getBlockState(pos), worldIn, pos));
        }
    }
