...
public class Chunk implements IChunkLighting, IChunkLightingData, ILightingEngineProvider {
    ...
    public Chunk(World worldIn, int x, int z) {
        ...
        this.lightingEngine = worldObj.getLightingEngine();
    }
    ...
    public IBlockState setBlockState(BlockPos pos, IBlockState state) {
        ...
        if (iblockstate == state) {
            ...
            if (extendedblockstorage == null) {
                ...
                extendedblockstorage = this.storageArrays[j >> 4] = initSection(j >> 4 << 4, !this.worldObj.provider.getHasNoSky());
            }
    ...
    private void checkSkylightNeighborHeight(WorldChunkSlice slice, int x, int z, int maxValue) {
        ...
        if (chunk == null) {
            log.warn("Chunk is null! x: {} z: {} maxValue: {}", x, z, maxValue);
            return;
        }
